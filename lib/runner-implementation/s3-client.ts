...
export const buildS3Client = (
  ...
  return new S3({
    endpoint: process.env[ENV_ENDPOINT] ?? options.endpoint,
    region: process.env[ENV_REGION] ?? options.region,
    credentials: provider,
    forcePathStyle:
      process.env[ENV_FORCE_PATH_STYLE] === "true" || options.forcePathStyle,
  });
};
